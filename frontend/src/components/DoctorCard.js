...
<div class="container mt-2">
  ...
  <div className="row g-0">
    ...
    <Link to={`/doctors/${props.doctor['id']}`} className="nonlink">
      <img src={props.doctor['profile'].picture} className="img-fluid rounded-circle img_height" alt="..."/>
  </Link>
    </div>
    <div className="col-md-8">
      <div className="card-body">
        <h5 className="card-title text-start">Dr.{props.doctor['user'].first_name} {props.doctor['user'].last_name}</h5>
        <p className=" text-start ">Pets doctor graduated from helwan university</p>
        <p className=" text-start"><Ratings rating={props.doctor['average_rate']}/></p>
        <p className=" text-start">{props.doctor['profile'].city},{props.doctor['profile'].country}</p>
        <p className=" text-start overdescription">{props.doctor['description']}</p>
...
{schedules.map((feed) => (
            <ScheduleCard key={feed.id} schedule={feed} doctor_id={props.doctor['id']}></ScheduleCard>
          ))}
